...
class DodgySquare:
    def __init__(self):
        ...
        self.enemy_frequency: int = 20  # @60 fps, low FQ = Lots; high FQ = Few
    ...
    def run(self):
        while True:
            for event in pygame.event.get():
                ...
                # Make sure player stays on the screen
                self.player_pos[0] = max(0, min(self.player_pos[0], self.screen_width - self.player_size))
                self.player_pos[1] = max(0, min(self.player_pos[1], self.screen_width - self.player_size))
            ...
            if not self.game_over:
                ...
                # Reset everything for thr next frame
